...
export class FollowRoute implements Route {
  ...
  private initializeRoutes() {
    this.router.post(`/users/:toBeFollowedId/follow`, auth, validate(followUserReqSchema), this.followController.followNewUser);
    this.router.post(`/users/:toBeUnfollowedId/unfollow`, auth, validate(unfollowUserReqSchema), this.followController.unfollowUser);
