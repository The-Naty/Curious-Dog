...
export interface IUserService {
  ...
  getUserStats(userId: number): Promise<{ username: string; profilePicture: string | null; numQuestions: number; numFollowers: number; numFollowing: number }>;
}
...
export class UserService implements IUserService {
  ...
  getUserStats = async (
    userId: number,
  ): Promise<{ username: string; profilePicture: string | null; numQuestions: number; numFollowers: number; numFollowing: number }> => {
    const data = await prisma.user.findUniqueOrThrow({
      where: { id: userId },
      select: { username: true, profilePicture: true, _count: { select: { receivedQuestions: true, followers: true, following: true } } },
    });
